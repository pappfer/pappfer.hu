...
### robots.txt
Generated by `build.js`. The wildcard group allows everything; AI crawlers,
answer engines and the AI opt-in tokens (`Google-Extended`, `Applebot-Extended`)
get their own explicit allow group, because several of them read only their own
group and ignore the wildcard one.
```
User-agent: *
Allow: /
