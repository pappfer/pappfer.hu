...
### robots.txt
Generated by `build.js`. One wildcard group that allows everything, plus comment
pointers to the machine-readable files. Do **not** add per-bot groups for GPTBot,
ClaudeBot and the rest: a crawler falls back to the `*` group when no group names
it (RFC 9309), so those groups grant access that already exists — and because
groups never merge, a later `Disallow:` in the wildcard group would silently not
apply to any bot that had its own.
```
User-agent: *
Allow: /
...
### IndexNow
Bing/Yandex/Seznam/Naver re-crawl on notification instead of on their own
